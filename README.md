...
时间，在满足全部条件时按设置使用系统锁屏或独立密码的应用锁屏。用户可以选择“任意人脸”或“仅本人”
...
在“设置 → 离席锁屏方式”选择“应用锁屏（独立密码）”，设置密码并保存应用。
密码不限长度和字符，支持单个字符、中文、空格及空密码。默认仍为 Windows 系统锁屏。
已有密码时，新密码留空会保留原密码；如需清空，勾选“设为空密码”。首次启用时
两次新密码都留空，也可以保存为空密码；空密码锁定后直接按 Enter 或点击“解锁”。
设置页以固定圆点显示“已设置密码”，并区分“未设置密码”和“已设为空密码”。
解锁状态下可以直接设置新密码、清空密码或切换锁屏方式，无需填写旧密码。
“预览应用锁屏（8 秒）”仅展示界面，会自动关闭，不拦截输入，也不保存密码。

锁屏采用山水静谧背景，显示实时时钟与日期。每块屏幕按自身尺寸独立裁切背景，
主屏显示密码框，副屏显示返回主屏解锁的提示。支持横竖屏、负坐标布局和主屏切换；
布局更新期间保留原有覆盖窗口，待新窗口准备好后替换，并保留已输入的密码。

“OLED 锁屏保护”默认开启：锁屏后键鼠闲置 3 分钟，所有显示器切换为黑底、正常亮度
的移动时钟，日期和装饰线随时钟一起移动，隐藏壁纸、密码框与鼠标指针。首次键鼠操作只恢复山水界面，
随后正常输入密码解锁。各屏幕内的时钟独立移动，显示器布局变化后继续保持保护。
这只是应用内画面变化，保持唤醒和后台任务不受影响；可在设置中关闭此选项。
降低静态画面亮度并移动显示内容有助于降低 OLED 残影风险，不能保证杜绝烧屏，
请保留显示器自身的保护功能（[厂商说明](https://www.asus.com/ca-en/support/faq/1048131/)）。

托盘图标右键菜单中的“应用锁屏”可立即手动锁定，不必等待离席检测。
即使自动锁屏方式设为 Windows 系统锁屏或监控已暂停，也可以手动使用；首次使用需
先在设置中保存应用密码。锁定前释放摄像头，解锁后恢复操作前的监控运行或暂停状态。

应用模式沿用无人及键鼠空闲阈值、5 秒预告和取消条件。锁定后释放摄像头，覆盖所有
显示器（含任务栏），拦截常见切窗快捷键及向其他窗口的输入，只能在应用内通过密码
...
**这是应用级遮挡，不等同于 Windows 安全锁屏。** Ctrl+Alt+Del 安全界面、任务管理器、
管理员、其他桌面、强制结束进程及同一账户修改程序或配置，都可能绕过保护。
程序崩溃后遮挡和输入过滤会失效；输入钩子也可能被系统移除。遗忘密码时可通过
Ctrl+Alt+Del 打开任务管理器结束进程并在解锁的 Windows 会话中重设本地设置；
这也是该模式安全强度有限的原因。对无人值守安全要求较高时应选择系统锁屏。

保持唤醒有助于避免普通空闲休眠和关闭屏幕，但不能阻止屏保、公司策略、动态锁、
手动睡眠、合盖、低电量或网络设备自身断线。长时间后台任务需在实际使用环境验证。
接口边界见微软的 [保持唤醒说明](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-setthreadexecutionstate)。

开发验证：`python -m unittest discover -s tests`；
`python tools/validate_app_lock.py --full-self-test` 在不切换用户桌面的独立测试桌面验证
真实窗口、输入钩子、密码解锁、显示器覆盖和现有自检，截图写入被忽略的 `dist` 目录。
其中多屏布局通过注入双屏/三屏、负坐标、竖屏、主屏/DPI 变更和枚举失败进行模拟；
真实混合缩放显示器及物理热插拔仍需在对应硬件上验收。
...
- 可选应用锁屏：全屏覆盖全部显示器，以独立密码解锁，监控期间保持唤醒，后台任务继续运行；
...
- **键鼠空闲后监测**：键盘或鼠标持续空闲达到设定时间后才打开摄像头，默认阈值为 20 秒；连续确认有人在场 10 秒后释放摄像头并进入待机，默认每 60 秒重新开启复查；一旦恢复键鼠操作，立即取消锁屏判定、清空调试画面并释放摄像头。

空闲后监测期间，OpenVINO 模型保持已加载状态，只开关摄像头设备，不反复重启
程序或重新编译模型。在场确认必须连续满足；无人脸、身份不符、摄像头读取失败或模型
推理异常都会重置这 10 秒计时。待机复查时如仍然在场，会再次确认后关闭；如未确认
在场，则保持摄像头开启并继续原有连续离席计时。键鼠状态无法读取时摄像头保持关闭且
禁止锁屏。该模式与自动多人脸
隐私模糊互斥，因为摄像头关闭期间无法可靠发现第二个人；但无需摄像头的手动毛玻璃
快捷键仍可使用。“任意人脸”和“仅本人”
...
监控首次确认在场时重新计时。鼠标悬停托盘图标可随时查看同一计时器的当前时长；
不足 1 分钟、等待首次确认、监控暂停或久坐提醒已关闭时会显示对应状态。
...
按全局快捷键可直接切换同一套 DWM 毛玻璃，默认组合键为 `Alt+B`。该功能不依赖
摄像头或人脸检测，因此在“键鼠空闲后监测”、摄像头待机以及监控暂停时仍然可用。
再次按下快捷键或快速甩动鼠标即可解除；托盘菜单中的“手动毛玻璃”也能执行相同
操作。快捷键可在设置页点击“录制”，然后直接按下包含 `Ctrl`、`Alt`、`Shift` 或
`Win` 的组合键，例如 `Alt+1`、`Ctrl+K` 或 `Ctrl+Shift+F8`。为避免正常输入文字
或数字时误触发，不能把单独的字母或数字注册为全局快捷键。保存时程序会先向 Windows 注册新组合键；如组合键
已被其他程序占用，将提示错误并继续保留原快捷键。

快捷键采用 Windows `RegisterHotKey`，不安装全局键盘钩子，不记录按键内容。
自定义组合必须包含至少一个 `Ctrl`、`Alt`、`Shift` 或 `Win` 修饰键，以及一个
字母、数字、`F1`-`F24` 或常用功能键。
...
调试控制台，在底部“摄像头”下拉框中选择本机摄像头；无需修改源码。
...
底部“摄像头”下拉框会枚举当前可用摄像头。选择新设备后，SeatSentinel 会
...
| `CAMERA_PRESENCE_AUTO_STANDBY_SECONDS` | `10.0` | 空闲后监测模式下连续确认在场多久后释放摄像头 |
| `CAMERA_PRESENCE_RECHECK_INTERVAL_SECONDS` | `60.0` | 确认在场并释放摄像头后的复查间隔 |
...
| `PRIVACY_BLUR_HOTKEY` | `Alt+B` | 手动切换毛玻璃的全局快捷键 |
