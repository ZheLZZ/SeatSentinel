...
仅本人模式下，如果连续确认本人身旁出现第二个人，SeatSentinel 会创建一个覆盖
全部显示器工作区的 Windows DWM Desktop Acrylic 合成窗口；任务栏保持清晰可用。
实时毛玻璃由 Windows 桌面合成器和 GPU 生成，SeatSentinel 不截取桌面，不读取、
缩放或高斯模糊桌面像素，也不建立桌面帧队列或刷新线程。用户甩动鼠标可立即解除；
若没有甩动，画面连续确认只剩已注册本人满 3 秒后也会自动解除。只剩一张陌生人脸
不会自动解除；暂停监控、Windows 锁屏或退出程序后，该原生窗口也会被销毁。
