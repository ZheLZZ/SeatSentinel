...
AwayLock 的设计目标是只在本机判断用户是否仍在电脑前，并在满足安全条件时
调用 Windows 锁屏。AwayLock 不提供云端服务，也不要求用户账户。
...
AwayLock 只做人脸存在检测，不进行身份识别，不建立人脸库，也不生成可用于
识别个人身份的人脸特征模板。
...
AwayLock 的正常监控代码不包含上传、远程推理或网络 API 调用。
...
OpenVINO 包含供其转换工具使用的可选遥测组件。AwayLock 不初始化这些转换
工具，并在导入 OpenVINO 前设置进程级 `DO_NOT_TRACK=1` 和
`SCARF_NO_ANALYTICS=1`。这些设置仅影响 AwayLock 进程，不修改用户的系统级
OpenVINO 偏好。

## 第三方组件

AwayLock 依赖 OpenVINO、OpenCV 等第三方开源组件。组件清单和许可信息见
`THIRD_PARTY_NOTICES.md`。第三方软件自身的行为和政策由其维护者负责。

## 隐私问题

如果发现 AwayLock 写入、保留或传输了本说明未披露的数据，请按照
